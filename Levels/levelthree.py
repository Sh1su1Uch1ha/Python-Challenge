...
list = [x for x in largerstring]
def isSandwiched(i):
    if i < 5 or i > len(list)-5 or list[i].isupper():
        return False
    sublist = list[i-3:i] + list[i+1:i+4]
    sublist2 = [list[i-4], list[i+4]]
    return all(x.isupper() for x in sublist) and all(x.islower() for x in sublist2)
finallist = [list[x] for x in range(len(list)) if isSandwiched(x)]
print finallist
